...
function Counter({ end, suffix = "", prefix = "" }) {
  ...
  useEffect(() => { if (!vis) return; let c = 0; const s = end / 60; const id = setInterval(() => { c += s; if (c >= end) { setN(end); clearInterval(id); } else setN(Math.floor(c)); }, 16); return () => clearInterval(id); }, [vis, end]);
  return <span ref={ref}>{prefix}{n}{suffix}</span>;
}

function Particles() {
  return <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
    {Array.from({ length: 18 }).map((_, i) => <div key={i} style={{ position: "absolute", width: Math.random() * 3 + 1, height: Math.random() * 3 + 1, backgroundColor: `rgba(99,102,241,${Math.random() * 0.25 + 0.08})`, borderRadius: "50%", left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, animation: `float ${Math.random() * 6 + 4}s ease-in-out infinite`, animationDelay: `${Math.random() * 4}s` }} />)}
  </div>;
}
        ...
        {steps.map((s, i) => {
          ...
          return (
            <div key={i} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} style={{
              flex: 1, textAlign: "center", opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(16px)",
              transition: `all 0.4s ease ${i * 0.08}s`, cursor: "default", position: "relative",
            }}>
              {/* connector */}
              {i > 0 && <div style={{ position: "absolute", left: 0, top: 19, width: "50%", height: 2,
                background: det ? `linear-gradient(90deg, rgba(99,102,241,0.3), ${color})` : past ? "rgba(239,68,68,0.15)" : "rgba(99,102,241,0.2)",
              }} />}
              {i < steps.length - 1 && <div style={{ position: "absolute", right: 0, top: 19, width: "50%", height: 2,
                background: det ? `linear-gradient(90deg, ${color}, rgba(239,68,68,0.15))` : past ? "rgba(239,68,68,0.15)" : "rgba(99,102,241,0.2)",
              }} />}
              <div style={{
                width: 40, height: 40, borderRadius: "50%", margin: "0 auto 8px", position: "relative", zIndex: 1,
                display: "flex", alignItems: "center", justifyContent: "center",
                background: det ? color : past ? "rgba(239,68,68,0.12)" : "rgba(99,102,241,0.1)",
                border: `2px solid ${det ? color : past ? "rgba(239,68,68,0.25)" : "rgba(99,102,241,0.2)"}`,
                transform: hov === i ? "scale(1.15)" : "scale(1)", transition: "all 0.3s",
                boxShadow: det ? `0 0 24px ${color}50` : "none",
              }}>
                <span style={{ color: det ? "#fff" : past ? "#ef4444" : "#a5b4fc" }}>
                  {det ? <I.Shield s={17} /> : past ? <I.X s={14} /> : s.icon}
                </span>
              </div>
...
function UseCase({ icon, title, threat, detection, impact, color, mitre, delay = 0 }) {
  ...
  return (
    <div ref={ref} onClick={() => setOpen(!open)} style={{
      ...
      border: `1px solid ${open ? color + "50" : "rgba(99,102,241,0.1)"}`,
      borderRadius: 16, padding: open ? 28 : 24, cursor: "pointer", transition: "all 0.35s",
      opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(24px)", transitionDelay: `${delay}s`,
      boxShadow: open ? `0 12px 40px ${color}12` : "none",
    }}>
      ...
      {open && (
        <div style={{ borderTop: `1px solid ${color}20`, paddingTop: 16, marginTop: 16, animation: "fadeSlide 0.3s ease" }}>
          ...
          {mitre && <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {mitre.map((t, i) => <span key={i} style={{ padding: "3px 10px", borderRadius: 100, background: "rgba(99,102,241,0.08)", color: "#a5b4fc", fontSize: 10, fontWeight: 600, fontFamily: "DM Sans" }}>{t}</span>)}
          </div>}
        </div>
      )}
    </div>
  ...
  return (
    <div ref={ref} style={{ position: "relative", width: "100%", height: 340, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {sources.map((s, i) => {
        const rad = (s.angle * Math.PI) / 180;
        const x = Math.cos(rad) * r, y = Math.sin(rad) * r;
        return <div key={i} style={{ position: "absolute", transform: `translate(${x}px, ${y}px)`, zIndex: 2, opacity: vis ? 1 : 0, transition: `all 0.5s ease ${i * 0.1 + 0.2}s`, display: "flex", flexDirection: "column", alignItems: "center" }}>
    ...
    <div style={{ fontFamily: "'Syne','DM Sans',system-ui,sans-serif", color: "#e2e8f0", background: "#0a0e1a", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "rgba(10,14,26,0.88)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(99,102,241,0.1)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 28 }}>
            ...
            <button className="cta-btn cta-primary" style={{ padding: "8px 20px", fontSize: 13 }}>Request Demo</button>
          </div>
        </div>
      </nav>
        ...
        <div className="sec" style={{ position: "relative", zIndex: 1, paddingTop: 120 }}>
          <div style={{ maxWidth: 720, animation: "slide-up 0.8s ease-out" }}>
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <button className="cta-btn cta-primary">Request Early Access <I.Arrow s={18} /></button>
              <button className="cta-btn cta-secondary">Watch 2-Min Overview</button>
            </div>
          </div>
          <div className="g4" style={{ marginTop: 80, borderTop: "1px solid rgba(99,102,241,0.15)", paddingTop: 40 }}>
            {[{ v: 80, s: "%", l: "Cost reduction vs. legacy SIEM" }, { v: 30, s: "+", l: "AI agents working 24/7" }, { p: "<", v: 1, s: "s", l: "Mean detection latency" }, { v: 5, s: "x", l: "Faster than ETL-based detection" }].map((x, i) =>
      ...
      <section id="shield" style={{ background: "linear-gradient(180deg,#0a0e1a 0%,#0f172a 100%)" }}>
        <div className="sec">
          ...
          {/* Shield Video Placeholder */}
          <div style={{ marginBottom: 48, borderRadius: 16, overflow: "hidden", border: "1px solid rgba(99,102,241,0.2)", background: "linear-gradient(135deg, rgba(15,23,42,0.9), rgba(30,41,59,0.5))", position: "relative" }}>
            <div style={{ aspectRatio: "16/9", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 50%, rgba(99,102,241,0.08) 0%, transparent 60%)" }} />
              {/* Play button */}
              <div style={{ width: 80, height: 80, borderRadius: "50%", background: "linear-gradient(135deg, #6366f1, #8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 0 40px rgba(99,102,241,0.4)", transition: "all 0.3s", position: "relative", zIndex: 1 }}>
                <svg width="32" height="32" viewBox="0 0 24 24" fill="white"><polygon points="8,5 19,12 8,19" /></svg>
              </div>
              <div style={{ marginTop: 20, fontFamily: "Syne", fontSize: 16, fontWeight: 700, color: "#c7d2fe", position: "relative", zIndex: 1 }}>Your Database Has a Blind Spot</div>
              <div style={{ fontFamily: "DM Sans", fontSize: 13, color: "#64748b", marginTop: 6, position: "relative", zIndex: 1 }}>See how AEGIS Shield detects query anomalies, data exfiltration, and credential misuse in real-time</div>
              <div style={{ marginTop: 12, padding: "4px 12px", borderRadius: 100, background: "rgba(99,102,241,0.1)", fontFamily: "DM Sans", fontSize: 11, color: "#a5b4fc", fontWeight: 600, position: "relative", zIndex: 1 }}>1:00 — Database Protection Overview</div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: 48, padding: "20px 28px", borderRadius: 12, background: "linear-gradient(90deg, rgba(16,185,129,0.06), rgba(99,102,241,0.06))", border: "1px solid rgba(16,185,129,0.15)", display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <I.Shield s={20} />
              <span style={{ fontFamily: "Syne", fontSize: 13, fontWeight: 700, color: "#6ee7b7" }}>Security & Privacy by Design</span>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {["SOC 2 Type II", "ISO 27001", "GDPR", "Data Residency"].map(b => <span key={b} style={{ padding: "3px 10px", borderRadius: 100, background: "rgba(110,231,183,0.08)", color: "#6ee7b7", fontSize: 10, fontWeight: 600, fontFamily: "DM Sans" }}>{b}</span>)}
            </div>
          </div>
      ...
      <section id="sentinel" style={{ background: "linear-gradient(180deg,#0f172a 0%,#0a0e1a 100%)" }}>
        <div className="sec">
          ...
          {/* Sentinel Video Placeholder */}
          <div style={{ marginBottom: 48, borderRadius: 16, overflow: "hidden", border: "1px solid rgba(139,92,246,0.2)", background: "linear-gradient(135deg, rgba(15,23,42,0.9), rgba(30,41,59,0.5))", position: "relative" }}>
            <div style={{ aspectRatio: "16/9", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 50%, rgba(139,92,246,0.08) 0%, transparent 60%)" }} />
              <div style={{ width: 80, height: 80, borderRadius: "50%", background: "linear-gradient(135deg, #8b5cf6, #a78bfa)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 0 40px rgba(139,92,246,0.4)", transition: "all 0.3s", position: "relative", zIndex: 1 }}>
                <svg width="32" height="32" viewBox="0 0 24 24" fill="white"><polygon points="8,5 19,12 8,19" /></svg>
              </div>
              <div style={{ marginTop: 20, fontFamily: "Syne", fontSize: 16, fontWeight: 700, color: "#c4b5fd", position: "relative", zIndex: 1 }}>The SIEM That Lives Where Your Data Lives</div>
              <div style={{ fontFamily: "DM Sans", fontSize: 13, color: "#64748b", marginTop: 6, position: "relative", zIndex: 1 }}>See how 30+ AI agents correlate across identity, endpoint, email, cloud, network, and database telemetry</div>
              <div style={{ marginTop: 12, padding: "4px 12px", borderRadius: 100, background: "rgba(139,92,246,0.1)", fontFamily: "DM Sans", fontSize: 11, color: "#c4b5fd", fontWeight: 600, position: "relative", zIndex: 1 }}>1:00 — AI SIEM on Any Datalake</div>
            </div>
          </div>

          {/* Correlation diagram + description */}
          <div className="g2" style={{ marginBottom: 48 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
              ...
              ].map((x, i) => <div key={i} style={{ display: "flex", gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ marginBottom: 48, padding: "20px 28px", borderRadius: 12, background: "linear-gradient(90deg, rgba(16,185,129,0.06), rgba(139,92,246,0.06))", border: "1px solid rgba(16,185,129,0.15)", display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
            ...
            <div style={{ height: 20, width: 1, background: "rgba(110,231,183,0.2)" }} />
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {["SOC 2 Type II", "ISO 27001", "GDPR", "Zero Trust", "Data Sovereignty"].map(b => <span key={b} style={{ padding: "3px 10px", borderRadius: 100, background: "rgba(110,231,183,0.08)", color: "#6ee7b7", fontSize: 10, fontWeight: 600, fontFamily: "DM Sans" }}>{b}</span>)}
            </div>
          </div>
      ...
      <section id="why-aegis" style={{ background: "linear-gradient(180deg, #0a0e1a 0%, #0f172a 100%)" }}>
        <div className="sec">
          <div style={{ textAlign: "center", marginBottom: 60 }}>
            ...
            <p style={{ color: "#94a3b8", maxWidth: 660, margin: "0 auto", fontFamily: "DM Sans", lineHeight: 1.6 }}>
              Lakehouse SIEMs bolt AI assistants onto data platforms. AEGIS was purpose-built for security investigation from day one — with capabilities no analytics-first platform can match.
            </p>
          </div>

          {/* Differentiation grid - 8 advantages */}
          <div className="g2" style={{ marginBottom: 48, gap: 16 }}>
            {[
              {
                badge: "INVESTIGATION-FIRST",
                title: "Built for DFIR, Not Dashboards",
                theirs: "Other platforms detect threats and surface alerts — then hand off to human analysts for manual investigation using query tools and dashboards.",
                ours: "AEGIS conducts full forensic investigations autonomously. Twenty specialized agents coordinate across detection, behavioral profiling, forensics, and response teams — working in parallel, not in sequence.",
                color: "#6366f1",
              },
              {
                badge: "CYBER INTELLIGENCE",
                title: "Threat-Informed, Not Just Anomaly-Detected",
                theirs: "Other SIEMs use statistical anomaly detection — they know something unusual happened, but can't contextualize it against real-world adversary campaigns, TTPs, or active threat intelligence.",
                ours: "AEGIS deploys a dedicated Cyber Intelligence Team: threat intel analysts, adversary TTP profilers, IOC enrichment agents, dark web monitors, and campaign trackers that correlate every alert against live threat intelligence — so you know not just what happened, but who's behind it and what they'll do next.",
                color: "#f472b6",
              },
              {
                badge: "SPECIALIZED AGENTS",
                title: "Domain Experts, Not Generic Copilots",
                theirs: "Lakehouse SIEMs offer AI assistants that help analysts write queries, author detection rules, and translate natural language to SQL — essentially productivity tools for existing workflows.",
                ours: "AEGIS deploys 30+ purpose-built agents with distinct domain expertise: network forensics, endpoint forensics, cloud forensics, cyber intelligence, threat intel correlation, regulatory compliance, evidence packaging. Each agent reasons autonomously within its domain, coordinated by a central orchestration layer that routes events and sequences response actions.",
                color: "#fbbf24",
              },
              {
                badge: "DATABASE-NATIVE",
                title: "Protection Inside the Database",
                theirs: "No lakehouse SIEM offers embedded database activity monitoring. They ingest database logs after the fact — creating a detection gap between when data is accessed and when the log arrives for analysis.",
                ours: "AEGIS Shield runs inside the database engine itself. It sees every query at execution time, classifies intent before results return, and detects anomalies with zero lag. No other SIEM architecture can match this.",
                color: "#34d399",
              },
              {
                badge: "EVIDENCE PACKAGING",
                title: "Regulatory-Ready, Not Dashboard-Ready",
                theirs: "Other platforms offer compliance dashboards and visualization tools. When the regulator calls, your team still spends days manually assembling evidence packages, cross-referencing logs, and preparing documentation.",
                ours: "AEGIS auto-generates framework-specific evidence packages — NYDFS 72-hour notification bundles, DORA incident reports, SOC 2 audit evidence — with full forensic timelines, risk scoring, and executive summaries. Ready to submit, not ready to query.",
                color: "#6ee7b7",
              },
              {
                badge: "ZERO-ETL",
                title: "Real-Time, Not Near-Real-Time",
                theirs: "Even \"decoupled\" lakehouse SIEMs batch-ingest data through ETL pipelines. \"Near-real-time\" means minutes of latency. When mean time to exploit is 1.6 days, minutes of detection delay compound into hours of response delay.",
                ours: "HTAP databases run analytical queries directly on live transactional data. AEGIS detects threats on the same write transaction that created the event — sub-second detection latency, not sub-minute.",
                color: "#60a5fa",
              },
              {
                badge: "VENDOR-AGNOSTIC",
                title: "Your Database, Your Choice",
                theirs: "Lakehouse SIEMs require you to run on their proprietary platform. Your security data, detection rules, ML models, and investigation history are locked into a single vendor — \"open formats\" notwithstanding.",
                ours: "AEGIS runs on any HTAP database: TiDB, SingleStore, CockroachDB, AlloyDB, and more. Switch vendors, run multi-cloud, maintain true data sovereignty. Your security platform should defend you, not lock you in.",
                color: "#fb923c",
              },
              {
                badge: "PARALLEL ORCHESTRATION",
                title: "All Teams, Simultaneously",
                theirs: "Traditional SIEM workflows are linear: detect → enrich → alert → triage → investigate → respond. Each step waits for the previous one. A complex investigation takes days.",
                ours: "AEGIS fires all four agent teams in parallel on every significant event: Detection scores the threat, Cyber Intelligence contextualizes the adversary, Investigation gathers forensic evidence, and Response prepares containment — all simultaneously, coordinated by the orchestration layer. Minutes, not days.",
                color: "#c4b5fd",
              },
            ].map((item, i) => {
              const [ref, vis] = useReveal(0.1);
              return <div ref={ref} key={i} style={{
                background: "linear-gradient(135deg, rgba(15,23,42,0.9), rgba(30,41,59,0.5))",
                border: `1px solid ${item.color}20`, borderRadius: 16, padding: 28, overflow: "hidden",
                opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(24px)",
                transition: `all 0.5s ease ${i * 0.06}s`, position: "relative",
              }}>
                {/* accent bar */}
                ...
                <div style={{ paddingLeft: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 10, alignItems: "flex-start" }}>
                    <div style={{ width: 20, height: 20, borderRadius: 5, background: "rgba(239,68,68,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>
                      <I.X s={12} />
                    </div>
                    <div style={{ fontFamily: "DM Sans", fontSize: 13, color: "#94a3b8", lineHeight: 1.55 }}>{item.theirs}</div>
                  </div>
                  <div style={{ display: "flex", gap: 6, alignItems: "flex-start" }}>
                    <div style={{ width: 20, height: 20, borderRadius: 5, background: "rgba(110,231,183,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2, color: "#6ee7b7" }}>
                      <I.Check s={12} />
                    </div>
          ...
          <div className="card" style={{ padding: 32, borderColor: "rgba(99,102,241,0.25)" }}>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "DM Sans", fontSize: 13 }}>
                <thead>
                  <tr>
                    {["Capability", "AEGIS", "Lakehouse SIEMs", "Legacy SIEMs", "DAM Tools"].map((h, i) => (
                      <th key={i} style={{ padding: "10px 14px", textAlign: i === 0 ? "left" : "center", borderBottom: "1px solid rgba(99,102,241,0.15)", color: i === 1 ? "#a5b4fc" : "#64748b", fontWeight: 700, fontSize: 12, letterSpacing: 0.5, fontFamily: "Syne" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={i} style={{ borderBottom: "1px solid rgba(99,102,241,0.06)" }}>
                      ...
                      {[1,2,3,4].map(j => (
                        <td key={j} style={{ padding: "10px 14px", textAlign: "center" }}>
                          {row[j] === true ? <span style={{ color: "#6ee7b7", fontWeight: 700 }}>✓</span> :
                           row[j] === false ? <span style={{ color: "#475569" }}>—</span> :
                           <span style={{ color: "#fbbf24", fontSize: 11 }}>{row[j]}</span>}
                        </td>
                      ))}
                    </tr>
                  ))}
      ...
      <section id="pricing" style={{ background: "linear-gradient(180deg,#0f172a,#0a0e1a)" }}>
        <div className="sec">
          ...
          <div className="g3" style={{ marginBottom: 48 }}>
            {[
            ...
            ].map((pl, i) => <div key={i} style={{ background: pl.hl ? "linear-gradient(135deg,rgba(99,102,241,0.15),rgba(139,92,246,0.1))" : "rgba(15,23,42,0.6)", border: `1px solid ${pl.hl ? "rgba(99,102,241,0.4)" : "rgba(99,102,241,0.1)"}`, borderRadius: 16, padding: 32, transition: "all 0.3s" }}>
              ...
              <button className={`cta-btn ${pl.hl ? "cta-primary" : "cta-secondary"}`} style={{ width: "100%", justifyContent: "center", marginTop: 20 }}>{pl.n === "Enterprise" ? "Contact Sales" : "Get Started"}</button>
            </div>)}
          </div>
          ...
          <div className="card" style={{ borderColor: "rgba(139,92,246,0.3)" }}>
            <div className="g2" style={{ gap: 48 }}>
              ...
              <div>
                ...
                {[["Splunk", "~$1.1M/yr", "#ef4444", 100], ["Microsoft Sentinel", "~$680K/yr", "#f59e0b", 62], ["Databricks Lakewatch", "~$400K/yr", "#fbbf24", 36], ["AEGIS on HTAP", "~$320K/yr", "#6ee7b7", 29]].map(([n, p, c, w], i) => <div key={i} style={{ marginBottom: 16 }}>
        ...
        <div className="sec" style={{ paddingTop: 60, paddingBottom: 60 }}>
          <div className="card" style={{ textAlign: "center", padding: "60px 40px", background: "linear-gradient(135deg,rgba(99,102,241,0.1),rgba(139,92,246,0.05))", borderColor: "rgba(99,102,241,0.3)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 16, flexWrap: "wrap" }}>
              <button className="cta-btn cta-primary">Partner With Us <I.Arrow s={18} /></button>
              <button className="cta-btn cta-secondary">Download Partner Brief</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer style={{ borderTop: "1px solid rgba(99,102,241,0.1)", background: "#0a0e1a" }}>
        <div className="sec" style={{ paddingTop: 40, paddingBottom: 40 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
            ...
            <div style={{ color: "#475569", fontSize: 13, fontFamily: "DM Sans" }}>&copy; 2026 Sentrixi. Security that lives where your data lives.</div>
          </div>
        </div>
      </footer>
    </div>

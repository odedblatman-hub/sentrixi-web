...
export default function InvestorsPage() {
  return (
    <main style={{ background: "#050609", color: "#e8edf5", minHeight: "100vh", fontFamily: "var(--font-sans, 'IBM Plex Sans', system-ui, sans-serif)", overflowX: "hidden" }}>
      <div style={{ position: "fixed", inset: 0, backgroundImage: "linear-gradient(rgba(167,139,250,0.025) 1px, transparent 1px), linear-gradient(90deg, rgba(167,139,250,0.025) 1px, transparent 1px)", backgroundSize: "80px 80px", pointerEvents: "none", zIndex: 0 }} />
      ...
      <section style={{ position: "relative", zIndex: 1, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "8rem 2rem 4rem" }}>
        ...
        <h1 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(3rem, 6.5vw, 6rem)", fontWeight: 400, lineHeight: 1.05, maxWidth: "960px", marginBottom: "2rem", color: "#f0f4fa", letterSpacing: "-0.01em" }}>
          Every infrastructure shift<br />
          <span style={{ color: ACCENT }}>created a new security category.</span><br />
          <span style={{ color: SILVER, fontSize: "0.7em" }}>The data layer is next.</span>
        </h1>

        <p style={{ fontSize: "1.1rem", color: "#7a92aa", maxWidth: "640px", lineHeight: 1.8, marginBottom: "3rem" }}>
          Perimeter shift created the firewall market. Cloud shift created CSPM. Identity shift created CIEM. The data execution layer — where breaches actually complete — has no equivalent security category. Until now.
        </p>

        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", justifyContent: "center" }}>
          <a href="#briefing" style={{ padding: "0.85rem 2.2rem", background: ACCENT, color: "#000", borderRadius: "6px", fontWeight: 700, textDecoration: "none", fontSize: "0.9rem", letterSpacing: "0.04em" }}>Request Investor Briefing</a>
          <a href="#thesis" style={{ padding: "0.85rem 2.2rem", border: "1px solid rgba(167,139,250,0.3)", color: ACCENT, borderRadius: "6px", textDecoration: "none", fontSize: "0.9rem" }}>Read the Thesis</a>
        </div>
      ...
      {/* THE SHIFT */}
      <section id="thesis" style={{ position: "relative", zIndex: 1, padding: "8rem 2rem", background: "rgba(5,4,12,0.8)", borderTop: "1px solid rgba(167,139,250,0.06)" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <SectionLabel>THE THESIS</SectionLabel>
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2.2rem, 4.5vw, 3.8rem)", fontWeight: 400, marginBottom: "1.5rem", maxWidth: "720px", lineHeight: 1.15 }}>
            Security always follows infrastructure.<br />
          ...
          <p style={{ color: "#7a92aa", maxWidth: "580px", lineHeight: 1.8, marginBottom: "5rem", fontSize: "1rem" }}>
            Each infrastructure layer that gained enterprise trust eventually required its own security primitive. Databases hold more sensitive data than any other layer in the stack — and remain the only major layer without embedded behavioral security.
          </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "0", position: "relative", marginBottom: "5rem" }}>
            {[
              ...
              <div key={item.shift} style={{ padding: "1.5rem 1.25rem", borderTop: `2px solid ${item.status === "open" ? ACCENT : "rgba(167,139,250,0.25)"}`, background: item.status === "open" ? DIM : "transparent", position: "relative" }}>
                ...
                <div style={{ fontSize: "0.75rem", color: item.status === "open" ? ACCENT : "#3a4a5a", fontFamily: "monospace" }}>{item.category}</div>
                {item.status === "open" && (
          ...
          <div style={{ padding: "2rem 2.5rem", border: "1px solid rgba(167,139,250,0.18)", borderLeft: `4px solid ${ACCENT}`, borderRadius: "8px", background: DIM, maxWidth: "740px" }}>
            <p style={{ color: "#e0daf8", fontSize: "1rem", lineHeight: 1.8 }}>
              <strong style={{ color: ACCENT }}>The category insight:</strong> No incumbent owns data-layer behavioral security. DAM tools are log-based and post-hoc. SIEMs are infrastructure-adjacent. The position is structurally unoccupied — and AEGIS is designed to own it.
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ position: "relative", zIndex: 1, padding: "8rem 2rem" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3.4rem)", fontWeight: 400, marginBottom: "1rem", lineHeight: 1.2 }}>This has happened before.</h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.5rem" }}>
            <AnalogyCard company="Databricks" move="Brought compute to the data layer" lesson="Created a $43B company by making the data layer programmable — not just storage." parallel="AEGIS makes the data layer observable and defensible." num="01" />
            <AnalogyCard company="Snowflake" move="Made data storage a platform" lesson="Turned a commodity infrastructure component into a $60B+ data cloud by adding intelligence and ecosystem." parallel="AEGIS adds security intelligence to the database as a native layer." num="02" />
            <AnalogyCard company="CrowdStrike" move="Repositioned endpoint as intelligence" lesson="Turned endpoint from a passive log collector into a real-time intelligence layer. $80B+ market cap." parallel="AEGIS repositions the database from passive storage to active detection surface." num="03" />
          </div>
        </div>
      </section>
      ...
      <section style={{ position: "relative", zIndex: 1, padding: "8rem 2rem", background: "rgba(5,4,12,0.8)", borderTop: "1px solid rgba(167,139,250,0.06)" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3.4rem)", fontWeight: 400, marginBottom: "4rem", lineHeight: 1.2 }}>Three signals converging simultaneously.</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.5rem", marginBottom: "3rem" }}>
            <SignalCard num="01" signal="Data volume is outpacing visibility infrastructure" body="Enterprise database instances grew 3× in five years. Security tooling didn't follow. The gap between data growth and security coverage is widening — not closing." stat="3×" statLabel="DB growth vs. tooling" />
            <SignalCard num="02" signal="Security is collapsing toward execution layers" body="Zero trust moved security to identity. AI-native threats are moving it to the execution layer. Perimeter defenses are structurally insufficient against credential-valid attacks." stat="83%" statLabel="Breaches use valid credentials" />
            <SignalCard num="03" signal="AI enables behavioral modeling at scale" body="Behavioral baselines that once required months of manual rule-writing can now be built in days. AI makes per-user, per-query profiling economically viable at enterprise scale." stat="90-Day" statLabel="Dynamic baselines" />
          </div>
          <div style={{ textAlign: "center", padding: "2.5rem", border: "1px solid rgba(167,139,250,0.15)", borderRadius: "12px", background: DIM }}>
            <p style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(1.4rem, 3vw, 2rem)", color: "#f0f4fa", lineHeight: 1.5 }}>
              "The database is the next control point.<br />
      ...
      <section style={{ position: "relative", zIndex: 1, padding: "8rem 2rem" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3.4rem)", fontWeight: 400, marginBottom: "1rem", lineHeight: 1.2 }}>
            Two distribution channels.<br />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem", marginBottom: "3rem" }}>
            <ModelCard channel="Direct" title="Enterprise Sales" desc="CISO and CTO buyers in regulated industries — financial services, healthcare, government. High ACV, long relationships, compliance-driven urgency." metrics={[["ACV range", "$24K – $500K+"], ["Sales motion", "Technical demo → POC → contract"], ["Buyer trigger", "Compliance deadline / breach event"]]} accent={ACCENT} />
            <ModelCard channel="OEM / Embed" title="Database Vendor Partnerships" desc="White-label or co-brand AEGIS as a native security feature. Revenue share model. Each partner multiplies reach across their entire customer base — without adding to Sentrixi's sales headcount." metrics={[["Revenue share", "Negotiated per partnership"], ["Distribution", "Partner's installed base"], ["Competitive moat", "Switching cost = platform switch"]]} accent="#00d4ff" highlight />
          </div>
      ...
      <section style={{ position: "relative", zIndex: 1, padding: "6rem 2rem", background: "rgba(5,4,12,0.8)", borderTop: "1px solid rgba(167,139,250,0.06)" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 400, marginBottom: "3rem", lineHeight: 1.2 }}>Land with Shield. Expand with Sentinel.</h2>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
            <ProductLine label="TIER 1 · LAND" name="AEGIS Shield" desc="Database-native behavioral detection. Replaces DAM tools. Designed for CISO/CTO buyers. Expands with every compliance trigger." arr="$24K – $500K+" accent="#00d4ff" />
            <ProductLine label="TIER 2 · EXPAND" name="AEGIS Sentinel" desc="Full AI SIEM on any datalake. 30+ agents. Consumption-based pricing. Expands revenue per customer 3–8× over Shield alone." arr="$48K platform + consumption" accent="#f59e0b" />
          </div>
        </div>
      </section>

      {/* BRIEFING FORM */}
      <section id="briefing" style={{ position: "relative", zIndex: 1, padding: "8rem 2rem", borderTop: "1px solid rgba(167,139,250,0.08)" }}>
        <div style={{ maxWidth: "560px", margin: "0 auto", textAlign: "center" }}>
          <SectionLabel>GET THE BRIEFING</SectionLabel>
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 400, marginBottom: "1rem", lineHeight: 1.2 }}>Request an investor briefing.</h2>
          <p style={{ color: "#7a92aa", marginBottom: "3rem", lineHeight: 1.7, fontSize: "0.95rem" }}>
            We'll share market analysis, architecture overview, business model detail, and current traction. No cold deck. A real conversation.
          </p>
          <InvestorForm />
        </div>
      </section>
...
function Metric({ value, label }) {
  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "2rem", color: "#e8edf5", marginBottom: "0.3rem" }}>{value}</div>
...
function AnalogyCard({ company, move, lesson, parallel, num }) {
  return (
    <div style={{ padding: "2rem", border: "1px solid rgba(167,139,250,0.1)", borderRadius: "12px", background: "rgba(10,8,20,0.6)", display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "1.6rem", color: "#f0f4fa" }}>{company}</div>
    ...
    <div style={{ padding: "1.75rem", border: "1px solid rgba(167,139,250,0.1)", borderRadius: "10px", background: "rgba(10,8,20,0.5)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1rem" }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "1.5rem", color: ACCENT }}>{stat}</div>
...
function ModelCard({ channel, title, desc, metrics, accent, highlight }) {
  return (
    <div style={{ padding: "2rem", border: `1px solid ${highlight ? accent + "35" : "rgba(167,139,250,0.1)"}`, borderRadius: "12px", background: highlight ? `${accent}08` : "rgba(10,8,20,0.5)", position: "relative" }}>
      {highlight && <div style={{ position: "absolute", top: "-1px", right: "1.5rem", background: accent, color: "#000", fontSize: "0.62rem", fontWeight: 700, letterSpacing: "0.12em", padding: "0.25rem 0.8rem", borderRadius: "0 0 6px 6px" }}>SCALE ENGINE</div>}
      <div style={{ fontSize: "0.65rem", letterSpacing: "0.15em", color: accent, opacity: 0.7, marginBottom: "0.5rem" }}>{channel}</div>
      <h3 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "1.5rem", fontWeight: 400, color: "#e0d8f8", marginBottom: "0.75rem" }}>{title}</h3>
...
function ProductLine({ label, name, desc, arr, accent }) {
  return (
    <div style={{ padding: "1.75rem", border: `1px solid ${accent}20`, borderRadius: "10px", background: `${accent}05` }}>
      ...
      <div style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "1.5rem", color: "#e0d8f8", marginBottom: "0.6rem" }}>{name}</div>
      <p style={{ fontSize: "0.83rem", color: "#6a7a88", lineHeight: 1.7, marginBottom: "1rem" }}>{desc}</p>
      <div style={{ fontSize: "0.78rem", color: "#5a6a80" }}>ACV: <span style={{ color: accent, fontWeight: 500 }}>{arr}</span></div>
    </div>
  );
}
...
function InvestorForm() {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          name: form.name,
          email: form.email,
          company: `${form.firm || "—"} — ${form.role || "Investor"}`,
          message: form.focus,
          type: "investor",
        }),
      });
  ...
  if (submitted) {
    return (
      <div style={{ padding: "3rem", border: "1px solid rgba(167,139,250,0.2)", borderRadius: "12px", background: DIM, textAlign: "center" }}>
        <div style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "1.5rem", color: ACCENT, marginBottom: "0.75rem" }}>Request sent.</div>
  ...
  return (
    <div style={{ display: "grid", gap: "1rem", textAlign: "left" }}>
      ...
      <button onClick={handleSubmit} disabled={status === "loading"} style={{ padding: "0.9rem", background: ACCENT, color: "#000", border: "none", borderRadius: "6px", fontWeight: 700, fontSize: "0.88rem", letterSpacing: "0.06em", cursor: status === "loading" ? "wait" : "pointer" }}>
        {status === "loading" ? "SENDING..." : "REQUEST INVESTOR BRIEFING →"}
      </button>
      {status === "error" && <p style={{ fontSize: "0.72rem", color: "#ff4444", textAlign: "center" }}>Something went wrong. Please try again.</p>}
...
function Footer() {
  return (
    <footer style={{ position: "relative", zIndex: 1, padding: "2rem 3rem", borderTop: "1px solid rgba(255,255,255,0.04)", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "0.78rem", color: "#3a4f62" }}>
      <Link href="/" style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "1rem", color: "#4a5070", textDecoration: "none" }}>SENTRIXI</Link>

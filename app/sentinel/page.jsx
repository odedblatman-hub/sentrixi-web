...
export default function SentinelPage() {
  ...
  return (
    <main style={{ background: "#050810", color: "#e8edf5", minHeight: "100vh", fontFamily: "var(--font-sans, 'IBM Plex Sans', system-ui, sans-serif)" }}>
      <div style={{ position: "fixed", inset: 0, backgroundImage: "linear-gradient(rgba(245,158,11,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(245,158,11,0.02) 1px, transparent 1px)", backgroundSize: "70px 70px", pointerEvents: "none", zIndex: 0 }} />
      <Nav />
      ...
      <section style={{ position: "relative", zIndex: 1, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "8rem 2rem 4rem" }}>
        ...
        {activeAudience === "vendor" ? (
          ...
          <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", justifyContent: "center" }}>
            <a href="#partner" style={{ padding: "0.75rem 2rem", background: ACCENT, color: "#000", borderRadius: "6px", fontWeight: 600, textDecoration: "none", fontSize: "0.88rem", letterSpacing: "0.04em" }}>Request Enterprise Demo</a>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1.5rem", marginTop: "5rem", padding: "2rem 2.5rem", border: "1px solid rgba(245,158,11,0.1)", borderRadius: "12px", background: "rgba(15,10,0,0.6)", maxWidth: "780px", width: "100%" }}>
          <StatBox value="30+" label="Specialized AI agents" />
          <StatBox value="<1s" label="Cross-source correlation" />
          <StatBox value="6+" label="Telemetry sources unified" />
          <StatBox value="70%" label="Cost reduction vs. Splunk" />
        </div>
      </section>
      ...
      <section style={{ position: "relative", zIndex: 1, padding: "5rem 2rem" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(1.8rem, 3.5vw, 2.8rem)", fontWeight: 400, marginBottom: "0.75rem", lineHeight: 1.2 }}>See Sentinel in action.</h2>
      ...
      <section id="embed" style={{ position: "relative", zIndex: 1, padding: "7rem 2rem", background: "rgba(15,10,0,0.7)", borderTop: "1px solid rgba(245,158,11,0.06)" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <SectionLabel>FOR DATABASE VENDORS</SectionLabel>
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3.2rem)", fontWeight: 400, marginBottom: "1rem", lineHeight: 1.2, maxWidth: "700px" }}>Turn security into a revenue line.</h2>
          <p style={{ color: "#7a92aa", maxWidth: "560px", lineHeight: 1.8, marginBottom: "4rem", fontSize: "0.95rem" }}>Security has always been external to the database. AEGIS Sentinel lets you embed it natively — transforming your platform into a security-first product and unlocking budgets your competitors can't reach.</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginBottom: "3rem" }}>
            <VendorCard title="White-Label or Co-Brand" body="Ship AEGIS Shield as a native security feature under your brand. Your customers get world-class behavioral detection. You own the relationship." icon="🏷️" />
            <VendorCard title="Revenue Share Model" body="Beyond white-label, participate in security revenue through a structured partnership model. Security budgets are 3–5× larger than infrastructure budgets." icon="💰" />
            <VendorCard title="Data Expansion" body="Security telemetry and behavioral signals drive significant increases in data ingestion and hot retention — translating directly to platform revenue." icon="📈" />
            <VendorCard title="Unlock Regulated Markets" body="Financial services, healthcare, government — these segments require database-native security. AEGIS removes the compliance blocker that has kept them out of your pipeline." icon="🔓" />
          </div>
          <div style={{ padding: "2rem", border: "1px solid rgba(245,158,11,0.15)", borderLeft: `4px solid ${ACCENT}`, borderRadius: "8px", background: "rgba(245,158,11,0.04)", maxWidth: "680px" }}>
            <p style={{ color: "#e8edf5", fontSize: "0.95rem", lineHeight: 1.7 }}><strong style={{ color: ACCENT }}>The shift:</strong> Database vendors have been infrastructure providers. AEGIS makes you a security provider — with the margins, retention, and enterprise relationships that come with it.</p>
          </div>
        </div>
      </section>

      {/* FOR ENTERPRISE SOC */}
      <section id="enterprise" style={{ position: "relative", zIndex: 1, padding: "7rem 2rem" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <SectionLabel>FOR ENTERPRISE SOC TEAMS</SectionLabel>
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3.2rem)", fontWeight: 400, marginBottom: "1rem", lineHeight: 1.2, maxWidth: "700px" }}>30+ AI agents replacing a 15-person SOC team.</h2>
          <p style={{ color: "#7a92aa", maxWidth: "560px", lineHeight: 1.8, marginBottom: "4rem", fontSize: "0.95rem" }}>Every significant security event triggers all four agent teams simultaneously — detection, cyber intelligence, forensics, and response — working in parallel, not in sequence.</p>
          <div id="agents"><AgentTeams /></div>
        </div>
      </section>
      ...
      <section style={{ position: "relative", zIndex: 1, padding: "5rem 2rem", background: "rgba(10,8,0,0.7)", borderTop: "1px solid rgba(245,158,11,0.06)" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(1.8rem, 3.5vw, 2.8rem)", fontWeight: 400, marginBottom: "1rem" }}>Connect to any datalake.</h2>
          <p style={{ color: "#7a92aa", marginBottom: "3rem", lineHeight: 1.7, fontSize: "0.9rem" }}>No rip-and-replace required. Route telemetry from any source into your existing infrastructure.</p>
      ...
      <section style={{ position: "relative", zIndex: 1, padding: "7rem 2rem" }}>
        <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 400, marginBottom: "4rem", lineHeight: 1.2 }}>Not just another SIEM.</h2>
      ...
      <section style={{ position: "relative", zIndex: 1, padding: "7rem 2rem", background: "rgba(10,8,0,0.7)", borderTop: "1px solid rgba(245,158,11,0.06)" }}>
        <div style={{ maxWidth: "860px", margin: "0 auto" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 400, marginBottom: "1rem" }}>Consumption-based. Predictable costs.</h2>
      ...
      {/* FORMS */}
      <section id="partner" style={{ position: "relative", zIndex: 1, padding: "7rem 2rem", borderTop: "1px solid rgba(245,158,11,0.08)" }}>
        <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem" }}>
            <div>
              <SectionLabel>DATABASE VENDORS</SectionLabel>
              <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "1.8rem", fontWeight: 400, marginBottom: "1rem", lineHeight: 1.2 }}>Start a partnership conversation.</h2>
              <p style={{ color: "#7a92aa", marginBottom: "2rem", fontSize: "0.88rem", lineHeight: 1.7 }}>We'll walk through the embed model, white-label options, and revenue structure — no commitment required.</p>
              <DualForm type="vendor" />
            </div>
            <div>
              <SectionLabel color={CYAN}>ENTERPRISE SOC</SectionLabel>
              <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "1.8rem", fontWeight: 400, marginBottom: "1rem", lineHeight: 1.2 }}>Request an enterprise demo.</h2>
              <p style={{ color: "#7a92aa", marginBottom: "2rem", fontSize: "0.88rem", lineHeight: 1.7 }}>We'll map Sentinel to your existing stack, telemetry sources, and SOC workflow — no generic pitch.</p>
...
function ToggleBtn({ label, active, onClick }) {
  return (
    <button onClick={onClick} style={{ padding: "0.7rem 1.5rem", background: active ? ACCENT : "transparent", color: active ? "#000" : "#7a92aa", border: "none", cursor: "pointer", fontSize: "0.85rem", fontWeight: active ? 600 : 400, fontFamily: "inherit", transition: "all 0.2s" }}>
      {label}
    </button>
  );
}

function VendorCard({ title, body, icon }) {
  return (
...
function AgentTeams() {
  const teams = [
    { name: "Detection Team", color: ACCENT, agents: ["Threat Detection Lead", "Behavioral Analytics", "Anomaly Scoring", "Detection Engineering", "Threat Intel Correlation", "Alert Prioritization"] },
    { name: "Cyber Intelligence", color: CYAN, agents: ["Threat Intel Lead", "Adversary TTP Profiler", "IOC Enrichment", "Dark Web Monitor", "Campaign Tracker", "Geo-Threat Analyst"] },
    { name: "Investigation Team", color: "#a78bfa", agents: ["DFIR Lead", "Forensic Analyst", "Network Forensics", "Endpoint Forensics", "Cloud Forensics", "Timeline Reconstructor"] },
    { name: "Response Team", color: "#34d399", agents: ["Incident Commander", "Containment Agent", "Evidence Packager", "Regulatory Compliance", "Executive Comms", "Post-Incident Analyst"] },
  ];
  return (
    <div>
...
function DualForm({ type }) {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          name: form.name,
          email: form.email,
          company: `${form.company}${form.role ? ` — ${form.role}` : ""}`,
          type: type === "vendor" ? "partner" : "briefing",
        }),
      });
  ...
  return (
    <div style={{ display: "grid", gap: "0.85rem" }}>
      ...
      <div>
        <label style={{ fontSize: "0.68rem", color: "#4a5a6a", letterSpacing: "0.08em", display: "block", marginBottom: "0.35rem" }}>{type === "vendor" ? "YOUR PLATFORM (optional)" : "YOUR STACK (optional)"}</label>
        <textarea style={{ ...inputStyle, height: "80px", resize: "vertical" }} value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} placeholder={type === "vendor" ? "Database platform, customer base, partnership interest..." : "Current SIEM, data sources, team size..."} />
      </div>
      <button onClick={handleSubmit} disabled={status === "loading"} style={{ padding: "0.8rem", background: accent, color: "#000", border: "none", borderRadius: "6px", fontWeight: 700, fontSize: "0.85rem", letterSpacing: "0.06em", cursor: status === "loading" ? "wait" : "pointer" }}>
        {status === "loading" ? "SENDING..." : type === "vendor" ? "REQUEST PARTNER BRIEF →" : "REQUEST ENTERPRISE DEMO →"}
      </button>
      {status === "error" && <p style={{ fontSize: "0.72rem", color: "#ff4444", textAlign: "center" }}>Something went wrong. Please try again.</p>}
      <p style={{ fontSize: "0.7rem", color: "#3a4a5a", textAlign: "center" }}>We respond within 1 business day. No spam. No sequences.</p>
    </div>
  );
}

    ...
    >
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: "4rem", flexWrap: "wrap", gap: "2rem" }}>
          <div>
            <div style={{ fontSize: "0.68rem", letterSpacing: "0.2em", color: baseAccent, opacity: 0.7, marginBottom: "1rem" }}>WHY NOW</div>
            <h2 style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(2rem, 4vw, 3.4rem)", fontWeight: 400, lineHeight: 1.15, color: "#f0f4fa", maxWidth: "600px" }}>
              The shift has already started.
            </h2>
          </div>
          <p style={{ fontSize: "0.9rem", color: "#5a7a8a", maxWidth: "340px", lineHeight: 1.7, textAlign: "right" }}>
        ...
        <div style={{ padding: "2rem 2.5rem", border: `1px solid ${baseAccent}20`, borderLeft: `4px solid ${baseAccent}`, borderRadius: "8px", background: `${baseAccent}06`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "2rem" }}>
          <p style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "clamp(1.1rem, 2.5vw, 1.6rem)", color: "#e8edf5", lineHeight: 1.4, maxWidth: "680px" }}>
            The database is the next control point in enterprise security infrastructure.
          </p>
          <a
            href={context === "ciso" ? "#demo" : "#briefing"}
            style={{ padding: "0.75rem 1.75rem", background: baseAccent, color: "#000", borderRadius: "6px", fontWeight: 700, textDecoration: "none", fontSize: "0.82rem", letterSpacing: "0.06em", flexShrink: 0 }}
          >
...
function SignalCard({ signal }) {
  return (
    <div style={{ padding: "1.75rem", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "10px", background: "rgba(5,10,20,0.6)", display: "flex", flexDirection: "column", gap: "0.85rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ fontSize: "0.65rem", letterSpacing: "0.15em", color: signal.accentColor, opacity: 0.5 }}>{signal.num}</div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontFamily: "var(--font-serif-display, 'DM Serif Display', serif)", fontSize: "1.5rem", color: signal.accentColor, lineHeight: 1 }}>{signal.stat}</div>
          <div style={{ fontSize: "0.62rem", color: "#3a4a5a", letterSpacing: "0.06em", marginTop: "0.2rem", maxWidth: "110px", textAlign: "right" }}>{signal.statLabel}</div>
        </div>
      </div>
      <h3 style={{ fontSize: "0.92rem", fontWeight: 600, color: "#c0d0e0", lineHeight: 1.4 }}>{signal.headline}</h3>
      <p style={{ fontSize: "0.82rem", color: "#5a6a78", lineHeight: 1.75 }}>{signal.body}</p>
      <div style={{ height: "2px", background: `linear-gradient(to right, ${signal.accentColor}50, transparent)`, borderRadius: "1px", marginTop: "auto" }} />
    </div>
  );
}
